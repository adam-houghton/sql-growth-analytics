...
CumulativeData AS (
    SELECT
        ...
        SUM(yearly_join_amount - yearly_leave_amount) OVER (PARTITION BY JoinYear ORDER BY year ASC) AS cumulative_subscription_count
    FROM
        YearAmounts
)
...
, NormalizationFactor AS (
    SELECT
        JoinYear,
        cumulative_subscription_count AS normalization_factor
    FROM
        CumulativeData
    WHERE
        age_in_years = 0
...
, NormalizedData AS (
    SELECT
        c.age_in_years,
        c.JoinYear,
        (c.cumulative_subscription_count / n.normalization_factor) * 100 AS normalized_cumulative_subscription_count
    FROM
        CumulativeData c
    JOIN
