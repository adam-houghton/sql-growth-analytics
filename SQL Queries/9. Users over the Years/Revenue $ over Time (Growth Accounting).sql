...
DateCounts AS (
    SELECT
        date,
        SUM(CASE WHEN `Table`.`Join Date` = date THEN `Table`.`Amount` ELSE 0 END) AS join_count,
        SUM(CASE WHEN `Table`.`Leave Date` = date THEN `Table`.`Amount` ELSE 0 END) AS leave_count
    FROM
        AllDates
    LEFT JOIN `Table`
